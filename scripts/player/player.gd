...
func _apply_look(dx: float, dy: float) -> void:
        ...
        yaw_node.rotation.y -= dx * sens
        pitch_node.rotation.x = clampf(pitch_node.rotation.x - dy * sens, -0.9, 0.55)

## Touch-facing camera orbit used by the on-screen controls (mobile).
func rotate_camera(delta_x: float, delta_y: float) -> void:
        _apply_look(delta_x, delta_y)
